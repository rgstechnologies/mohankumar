...
export function EstimatesTab({
  ...
  async function withPdf(est: EstimateView, fn: (p: string) => Promise<void>) {
    try {
      await fn(`/companies/${companyId}/estimates/${est.id}/pdf`);
    } catch {
      toast(t('toast.pdfFailed'), 'error');
    }
  }

  async function setStatus(est: EstimateView, status: 'ACCEPTED' | 'DECLINED' | 'OPEN') {
    try {
      await api.post(`/companies/${companyId}/estimates/${est.id}/status`, { status });
      await onChanged();
  ...
  async function convert(est: EstimateView) {
    ...
    if (!ok) return;
    try {
      const updated = await api.post<EstimateView>(
        `/companies/${companyId}/estimates/${est.id}/convert`,
      );
      await onChanged();
  ...
  async function cancel(est: EstimateView) {
    const ok = await confirm({
    ...
    if (!ok) return;
    try {
      await api.post(`/companies/${companyId}/estimates/${est.id}/cancel`);
      await onChanged();
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {canBill && (
          <Button
            variant="primary"
            onClick={() => router.push(`/company/${companyId}/estimates/new`)}
          >
            {t('newEstimate')}
          </Button>
        )}
      ...
      <Card>
        {estimates.length === 0 ? (
          <EmptyState
            ...
            action={
              canBill && (
                <Button onClick={() => router.push(`/company/${companyId}/estimates/new`)}>
                  {t('empty.cta')}
                </Button>
              )
            }
                      ...
                      <RowActions
                        ...
                        actions={[
                          ...
                          ...(canBill && est.status !== 'CONVERTED' && est.status !== 'CANCELLED'
                            ? [{ label: tc('edit'), onClick: () => router.push(`/company/${companyId}/estimates/${est.id}/edit`) }]
                            : []),
                          ...
                          est.isTaxed &&
                          !est.isBooked &&
                          (est.status === 'OPEN' || est.status === 'ACCEPTED')
                          ...
                          ...(canCancel && est.status !== 'CONVERTED' && est.status !== 'CANCELLED'
                            ...
                            : []),
                          ...(canCancel
                            ? [{ label: tc('delete'), onClick: () => void del(`/companies/${companyId}/estimates/${est.id}`), tone: 'danger' as const }]
                            : []),
                        ]}
                      />
                    </td>
