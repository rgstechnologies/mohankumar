...
import { inr, type PurchaseEstimateView, type ItemRow, type PartyRow } from '@/lib/accounting';
...
export function PurchaseEstimatesTab({
  companyId,
  parties,
  items,
  branches,
  ...
  estimates: PurchaseEstimateView[];
  canBill: boolean;
  canCancel: boolean;
  onChanged: () => Promise<void>;
}) {
  const t = useTranslations('purchaseEstimates');
  const tc = useTranslations('common');
  const router = useRouter();

  const table = useTable(
    estimates,
    (e) => `${e.estimateNo} ${e.party.name} ${e.status}`,
  );
  const { toast, confirm } = useFeedback();
  const del = useDeleteDocument(onChanged);

  async function withPdf(est: PurchaseEstimateView, fn: (p: string) => Promise<void>) {
    try {
      await fn(`/companies/${companyId}/purchase-estimates/${est.id}/pdf`);
    } catch {
      toast(t('toast.pdfFailed'), 'error');
    }
  }

  async function setStatus(est: PurchaseEstimateView, status: 'ACCEPTED' | 'DECLINED' | 'OPEN') {
    try {
      await api.post(`/companies/${companyId}/purchase-estimates/${est.id}/status`, { status });
      await onChanged();
  ...
  async function convert(est: PurchaseEstimateView) {
    const ok = await confirm({
      title: t('convertDialog.title'),
      body: t('convertDialog.body', { no: est.estimateNo }),
      confirmLabel: t('convertDialog.confirm'),
    });
    if (!ok) return;
    try {
      const updated = await api.post<PurchaseEstimateView>(
        `/companies/${companyId}/purchase-estimates/${est.id}/convert`,
      );
      await onChanged();
      toast(t('toast.converted', { no: updated.bill?.billNo ?? '' }));
    } catch (err) {
      toast(err instanceof ApiError ? err.message : t('toast.convertFailed'), 'error');
    }
  }

  async function cancel(est: PurchaseEstimateView) {
    const ok = await confirm({
    ...
    if (!ok) return;
    try {
      await api.post(`/companies/${companyId}/purchase-estimates/${est.id}/cancel`);
      await onChanged();
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {canBill && (
          <Button
            variant="primary"
            onClick={() => router.push(`/company/${companyId}/purchase-estimates/new`)}
          >
            {t('newEstimate')}
          </Button>
        )}
      ...
      <Card>
        {estimates.length === 0 ? (
          <EmptyState
            ...
            action={
              canBill && (
                <Button onClick={() => router.push(`/company/${companyId}/purchase-estimates/new`)}>
                  {t('empty.cta')}
                </Button>
              )
            }
              ...
              <tbody>
                {table.rows.map((est) => (
                  <tr key={est.id} className="border-b border-line last:border-0 align-top hover:bg-subtle">
                    <td className="py-2 font-mono text-xs">{est.estimateNo}</td>
                      ...
                      <RowActions
                        ...
                        actions={[
                          ...
                          ...(canBill && est.status !== 'CONVERTED' && est.status !== 'CANCELLED'
                            ? [{ label: tc('edit'), onClick: () => router.push(`/company/${companyId}/purchase-estimates/${est.id}/edit`) }] : []),
                          ...
                          ...(canCancel && est.status !== 'CONVERTED' && est.status !== 'CANCELLED'
                            ? [{ label: tc('cancel'), onClick: () => void cancel(est), tone: 'danger' as const, divider: true as const }] : []),
                          ...(canCancel
                            ? [{ label: tc('delete'), onClick: () => void del(`/companies/${companyId}/purchase-estimates/${est.id}`), tone: 'danger' as const }] : []),
                        ]}
                      />
                    </td>
                  </tr>
